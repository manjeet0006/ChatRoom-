...
const SignUpPage = () => {
  ...
  const validateFrom = () => {
    ...
    const allowedDomains = ["gmail.com", "yahoo.com", "vgu.ac.in"];
    const emailDomain = formData.email.split("@")[1];
  
    if (!formData.fullName.trim()) return toast.error("Full name is required");
    if (!formData.email.trim()) return toast.error("Email is required");
    if (!emailRegex.test(formData.email)) return toast.error("Invalid email format");
    if (!allowedDomains.includes(emailDomain)) return toast.error("Please use a valid email provider (gmail, yahoo, or outlook)");
  ...
  return (
    <div className='min-h-screen grid lg:grid-cols-2'>
      ...
      <AuthImagePattern
        ...
        subtitle="connect with friends, share moments, and stay in touch woth your loved ones."
